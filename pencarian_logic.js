...
let desktopPencarianSubMode = 'global';
let mobilePencarianSubMode = 'menu';
...
document.addEventListener('DOMContentLoaded', async () => {
    initModernLayout({ id: 'pencarian', title: 'PENCARIAN ITEM', url: 'pencarian.html' });
    await muatDataStok();
});

window.muatDataStok = async function() {
    const isMobile = window.innerWidth < 640;
    const containerId = isMobile ? 'view-pencarian-mobile' : 'view-pencarian-desktop';
    const container = document.getElementById(containerId);
    
    if(container) {
        container.classList.remove('hidden');
        container.innerHTML = `<div class="p-10 text-center"><i data-lucide="loader-2" class="animate-spin w-6 h-6 mx-auto mb-2 text-slate-500"></i><p class="font-medium text-slate-500">Menghubungkan ke database...</p></div>`;
        lucide.createIcons();
    }

    try {
        const [stokGlobalData, resAktual] = await Promise.all([
        ...
        updateDatalists();
        initPencarianMode(isMobile);
    } catch(e) { 
        if(container) container.innerHTML = `<div class="p-10 text-center text-red-500 font-medium">Gagal mengolah data: ${e.message}</div>`; 
    }
};

function updateDatalists() {
    let namaSet = new Set(), pjgSet = new Set(), gradeSet = new Set();
    let dusSet = new Set(), shadingSet = new Set(), areaSet = new Set();
    let custSet = new Set(), estSet = new Set();

    dataKSArea.forEach(r => {
        if(r.nama) namaSet.add(r.nama);
        if(r.pjg) pjgSet.add(r.pjg);
        if(r.grade) gradeSet.add(r.grade);
        if(r.dus) dusSet.add(r.dus);
        if(r.shading) shadingSet.add(r.shading);
        if(r.area) areaSet.add(r.area);
        if(r.po_aktual) custSet.add(r.po_aktual);
        if(r.customer_estimasi) estSet.add(r.customer_estimasi);
    });

    const fillList = (id, setArr) => {
        const dl = document.getElementById(id);
        if(dl) {
            dl.innerHTML = '';
            Array.from(setArr).sort().forEach(val => {
    ...
    fillList('dl-nama-item', namaSet);
    fillList('dl-panjang', pjgSet);
    fillList('dl-grade', gradeSet);
    fillList('dl-dus', dusSet);
    fillList('dl-shading', shadingSet);
    fillList('dl-area', areaSet);
    fillList('dl-cust-aktual', custSet);
    fillList('dl-cust-estimasi', estSet);
}
...
window.initPencarianMode = function(isMobile) {
    ...
    document.getElementById('view-pencarian-desktop').classList.toggle('hidden', isMobile);
    document.getElementById('view-pencarian-mobile').classList.toggle('hidden', !isMobile);
...
window.toggleMobileFilterBox = function() {
    ...
    if (body) body.classList.toggle('hidden', !isMobileFilterOpen);
    if (icon) icon.style.transform = isMobileFilterOpen ? 'rotate(0deg)' : 'rotate(180deg)';
    if (lbl) lbl.innerText = isMobileFilterOpen ? 'Tutup' : 'Buka';
};
...
window.bukaDetailQRGlobal = function(encodedData) {
    ...
    window.activeDetailQRs = matchedQRs.map(g => g.qrcode);
...
window.salinSemuaQRDetail = function() {
    if(!window.activeDetailQRs || window.activeDetailQRs.length === 0) return alert("Tidak ada QR Code untuk disalin.");
    navigator.clipboard.writeText(window.activeDetailQRs.join('\n')).then(() => {
        alert(`✅ ${window.activeDetailQRs.length} QR Code berhasil disalin ke clipboard!`);
    });
};
        ...
        qrs.forEach(code => {
            ...
            if (g) {
                ...
                searchedQRResults.push({
                    ...
                    panjang: '-', grade: '-', dus: '-', shading: '-',
                    customerAktual: '-', customerEstimasi: '-', keterangan: '-',
                ...
                <div id="body-mobile-filter" class="p-4 space-y-3 ${isMobileFilterOpen ? '' : 'hidden'}">
                    <div class="grid grid-cols-2 gap-2">
                        <div class="col-span-2">
                            ...
                            <input type="text" id="m-f-nama" list="dl-nama-item" value="${globalSearchFilters.nama}" autocomplete="off" class="w-full p-2.5 border border-slate-300 rounded-xl text-xs font-bold bg-slate-50 outline-none uppercase focus:border-blue-500">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase text-slate-500 mb-1">Panjang</label>
                            <input type="text" id="m-f-pjg" list="dl-panjang" value="${globalSearchFilters.pjg}" autocomplete="off" placeholder="Cth: 4M" class="w-full p-2.5 border border-slate-300 rounded-xl text-xs font-bold bg-slate-50 outline-none uppercase focus:border-blue-500">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase text-slate-500 mb-1">Grade</label>
                            <input type="text" id="m-f-grade" list="dl-grade" value="${globalSearchFilters.grade}" autocomplete="off" placeholder="Cth: BAGUS" class="w-full p-2.5 border border-slate-300 rounded-xl text-xs font-bold bg-slate-50 outline-none uppercase focus:border-blue-500">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase text-slate-500 mb-1">Dus</label>
                            <input type="text" id="m-f-dus" list="dl-dus" value="${globalSearchFilters.dus}" autocomplete="off" class="w-full p-2.5 border border-slate-300 rounded-xl text-xs font-bold bg-slate-50 outline-none uppercase focus:border-blue-500">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase text-slate-500 mb-1">Shading</label>
                            <input type="text" id="m-f-shading" list="dl-shading" value="${globalSearchFilters.shading}" autocomplete="off" class="w-full p-2.5 border border-slate-300 rounded-xl text-xs font-bold bg-slate-50 outline-none uppercase focus:border-blue-500">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase text-slate-500 mb-1">Area</label>
                            <input type="text" id="m-f-area" list="dl-area" value="${globalSearchFilters.area}" autocomplete="off" class="w-full p-2.5 border border-slate-300 rounded-xl text-xs font-bold bg-slate-50 outline-none uppercase focus:border-blue-500">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase text-slate-500 mb-1">Customer Aktual</label>
                            <input type="text" id="m-f-cust" list="dl-cust-aktual" value="${globalSearchFilters.cust}" autocomplete="off" class="w-full p-2.5 border border-slate-300 rounded-xl text-xs font-bold bg-slate-50 outline-none uppercase focus:border-blue-500">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black uppercase text-slate-500 mb-1">Customer Estimasi</label>
                            <input type="text" id="m-f-cust-est" list="dl-cust-estimasi" value="${globalSearchFilters.est}" autocomplete="off" class="w-full p-2.5 border border-slate-300 rounded-xl text-xs font-bold bg-slate-50 outline-none uppercase focus:border-blue-500">
                        </div>
                    </div>
        ...
        html += `
            <div class="bg-white p-3.5 rounded-xl border border-slate-200 shadow-sm grid grid-cols-4 lg:grid-cols-8 gap-2 shrink-0">
                <div>
                    ...
                    <input type="text" id="pc-f-nama" list="dl-nama-item" value="${globalSearchFilters.nama}" autocomplete="off" placeholder="Ketik Item..." class="w-full p-2 text-xs font-bold border border-slate-300 rounded-lg bg-slate-50 outline-none uppercase focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-[9px] font-black uppercase text-slate-400 mb-1">Panjang</label>
                    <input type="text" id="pc-f-pjg" list="dl-panjang" value="${globalSearchFilters.pjg}" autocomplete="off" placeholder="Cth: 4M" class="w-full p-2 text-xs font-bold border border-slate-300 rounded-lg bg-slate-50 outline-none uppercase focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-[9px] font-black uppercase text-slate-400 mb-1">Grade</label>
                    <input type="text" id="pc-f-grade" list="dl-grade" value="${globalSearchFilters.grade}" autocomplete="off" placeholder="Cth: BAGUS" class="w-full p-2 text-xs font-bold border border-slate-300 rounded-lg bg-slate-50 outline-none uppercase focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-[9px] font-black uppercase text-slate-400 mb-1">Dus</label>
                    <input type="text" id="pc-f-dus" list="dl-dus" value="${globalSearchFilters.dus}" autocomplete="off" placeholder="Ketik Merk..." class="w-full p-2 text-xs font-bold border border-slate-300 rounded-lg bg-slate-50 outline-none uppercase focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-[9px] font-black uppercase text-slate-400 mb-1">Shading</label>
                    <input type="text" id="pc-f-shading" list="dl-shading" value="${globalSearchFilters.shading}" autocomplete="off" placeholder="Ketik Shading..." class="w-full p-2 text-xs font-bold border border-slate-300 rounded-lg bg-slate-50 outline-none uppercase focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-[9px] font-black uppercase text-slate-400 mb-1">Area</label>
                    <input type="text" id="pc-f-area" list="dl-area" value="${globalSearchFilters.area}" autocomplete="off" placeholder="Ketik Area..." class="w-full p-2 text-xs font-bold border border-slate-300 rounded-lg bg-slate-50 outline-none uppercase focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-[9px] font-black uppercase text-slate-400 mb-1">Cust Aktual</label>
                    <input type="text" id="pc-f-cust" list="dl-cust-aktual" value="${globalSearchFilters.cust}" autocomplete="off" placeholder="Ketik Cust..." class="w-full p-2 text-xs font-bold border border-slate-300 rounded-lg bg-slate-50 outline-none uppercase focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-[9px] font-black uppercase text-slate-400 mb-1">Cust Estimasi</label>
                    <input type="text" id="pc-f-cust-est" list="dl-cust-estimasi" value="${globalSearchFilters.est}" autocomplete="off" placeholder="Ketik Est..." class="w-full p-2 text-xs font-bold border border-slate-300 rounded-lg bg-slate-50 outline-none uppercase focus:border-blue-500">
                </div>
            </div>
