...
window.muatDataStok = async function() {
    ...
    try {
        ...
        window.dataKSQR = window.stokQRRaw.filter(r => r.kondisi !== 'NONAKTIF').map(r => {
            let p = r.id_sku ? r.id_sku.split('_') : [];
            let t = window.translateBarcode(r.qrcode);
            return {
        ...
        window.dataKSArea = window.stokAktualRaw.filter(a => a.kondisi !== 'NONAKTIF').map(a => {
        ...
        window.dataKSNonaktif = window.stokLembaranRaw; 
...
window.renderTabel = function() {
    ...
    if(window.modeKS === 'qr') {
        thead.innerHTML = `
            <tr>
                ...
                ${window.thSort(12, 'Customer Aktual', 'col-po')}
                ${window.thSort(13, 'Keterangan', 'col-ket')}
            </tr>`;
        
        if(window.dataKSQR.length === 0) { tbody.innerHTML = `<tr id="empty-row-ks"><td colspan="14" class="p-8 text-center font-medium text-slate-400">Tidak ada stok tersimpan.</td></tr>`; return; }
        ...
        if(window.dataKSArea.length === 0) { tbody.innerHTML = `<tr id="empty-row-ks"><td colspan="12" class="p-8 text-center font-medium text-slate-400">Tidak ada stok tersimpan.</td></tr>`; return; }

        tbody.innerHTML = window.dataKSArea.map((r) => {
            const safeQRs = JSON.stringify(r.qrcodes).replace(/"/g, "&quot;");
            
            let isProcessing = window.processedGantiKeys.has(`${r.id_sku_base}_${r.customer_estimasi}_${r.area}`);
        ...
        if(window.dataKSGlobal.length === 0) { tbody.innerHTML = `<tr id="empty-row-ks"><td colspan="12" class="p-8 text-center font-medium text-slate-400">Tidak ada stok tersimpan.</td></tr>`; return; }

        tbody.innerHTML = window.dataKSGlobal.map((r) => {
            let checkKey = `${r.nama}_${r.pjg}_${r.grade}_${r.dus}_${r.shading}_${r.ket}_${r.po}`;
            let isProcessing = window.processedGlobalKeys.has(`${checkKey}_${r.customer_estimasi}`);
            let iconGanti = isProcessing ? `<div class="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 z-10 bg-white rounded-full shadow-md border border-blue-300 p-1 text-blue-600" title="Sedang diproses ganti label"><i data-lucide="arrow-right-left" class="w-3 h-3"></i></div>` : '';
    ...
    else if (window.modeKS === 'nonaktif') {
        thead.innerHTML = `
            <tr>
                <th class="hdr-std w-10 col-cb text-center sticky-col">
                    <button id="btn-select-all" onclick="window.cycleSelectAll()" class="w-4 h-4 border border-slate-400 rounded flex items-center justify-center bg-white transition mx-auto" title="Klik untuk Pilih Semua"></button>
                ...
                ${window.thSort(9, 'Customer Aktual', 'col-po')}
                ${window.thSort(10, 'Customer Estimasi', 'col-estimasi')}
                ${window.thSort(11, 'Keterangan', 'col-ket')}
            </tr>`;
        
        if(window.dataKSNonaktif.length === 0) { tbody.innerHTML = `<tr id="empty-row-ks"><td colspan="12" class="p-8 text-center font-medium text-slate-400">Tidak ada data stok nonaktif.</td></tr>`; return; }

        tbody.innerHTML = window.dataKSNonaktif.map((r) => {
            return `
            <tr class="${rowClassBase} !bg-red-100 !text-red-900 font-bold is-nonaktif">
                ...
                <td class="px-4 py-3 font-medium text-left col-pjg" data-search="${r.panjang || '-'}">${r.panjang || '-'}</td>
...
window.siapkanGantiPO = function(context) {
    let checkboxes = [];
    if(context === 'main') {
        if(window.modeKS === 'global' || window.modeKS === 'nonaktif') return;
