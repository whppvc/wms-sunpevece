...
function addRow(area, code, isDuplicate = false) {
    ...
    div.className = `row-item ${rowClass} border-b border-slate-400 p-2 relative shadow-sm transition w-full flex shrink-0`; 
    ...
    div.innerHTML = `
        ...
        <div class="flex-1 flex flex-col gap-0.5 w-full min-w-0 py-0.5">
...
async function VerifikasiDanCek() {
    ...
    try {
        ...
        rows.forEach(r => {
            ...
            if(kodeSpan.innerText.includes('DUPLIKAT SCAN')) { hasError = true; } 
...
async function holdLangsir() {
    ...
    checkedBoxes.forEach(cb => {
        const div = cb.closest('.row-item');
        payloadUpload.push({ qrcode: div.querySelector('.qr-val').innerText, troli: div.querySelector('.troli-cell').innerText, area: div.querySelector('.area-cell').innerText, keterangan: `STBJ: ${div.querySelector('.stbj-val').innerText} | KODE: ${div.querySelector('.kode-val').innerText} | Ket: ${div.querySelector('.ket-cell').innerText}`, pic_input: user.username });
    });
    try {
