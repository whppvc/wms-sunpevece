...
function addRow(troli, area, code) {
    ...
    tr.innerHTML = `
        ...
        <td class="p-3 col-tgl text-slate-700">${td.tglProduksi}</td>
        <td class="p-3 col-mesin text-slate-700">${td.mesin}</td>
        <td class="p-3 col-shift text-slate-700 border-r border-slate-200">${td.shift}</td>
        <td class="p-3 font-black text-blue-700 col-jenis">${td.jenisItem}</td>
        <td class="p-3 font-bold text-slate-800 col-nama">${td.namaItem}</td>
        <td class="p-3 font-bold text-slate-600 col-pjg">${td.panjang}</td>
        <td class="p-3 font-bold text-slate-800 col-grade">${td.grade}</td>
        <td class="p-3 text-slate-800 col-dus">${td.dus}</td>
        <td class="p-3 text-slate-700 col-shading border-r border-slate-200">${td.shading}</td>
...
async function validasiSTBJ() {
    ...
    if(adaInv) alert("Ada yang belum STBJ (Oren)."); else alert("Semua sudah STBJ.");
...
async function saveToSupabase() {
    ...
    if (error || (data && data.startsWith('ERROR'))) { alert("GAGAL: " + (error ? error.message : data)); btn.innerHTML = original; btn.disabled = false; return; }
    alert(`BERHASIL! ${arrFisik.length} kardus tersimpan.`);
