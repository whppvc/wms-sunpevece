...
window.muatData = async function() {
    ...
    tbody.innerHTML = `<tr><td colspan="12" class="p-10 text-center"><i data-lucide="loader-2" class="animate-spin w-6 h-6 mx-auto mb-2 text-slate-500"></i><p class="font-medium text-slate-500">Menarik Data...</p></td></tr>`;
    lucide.createIcons();
    ...
    try {
        ...
        tbody.innerHTML = `<tr><td colspan="12" class="p-10 text-center text-red-500 font-bold">Gagal: ${e.message}</td></tr>`; 
    }
};
    ...
    return `<th class="hdr-std ${cls} select-none group">
        <div class="flex items-center justify-between w-full min-w-max gap-4">
            ...
            <div class="flex items-center gap-1 shrink-0">
                ...
                <button onclick="openColumnFilter(event, '${cls}', '${label}')" class="p-1 hover:bg-slate-700 rounded transition"><i data-lucide="filter" class="w-3.5 h-3.5 filter-icon opacity-40 hover:opacity-100 text-white"></i></button>
            </div>
        </div>
    </th>`;
};
...
window.renderTabel = function() {
    ...
    if(rawData.length === 0) { tbody.innerHTML = `<tr><td colspan="13" class="p-8 text-center font-medium text-slate-400">Tidak ada data.</td></tr>`; document.getElementById('lbl-total').innerText = 0; return; }
