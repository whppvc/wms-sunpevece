...
function renderHeaderDanTabel() {
    ...
    if(modeSekarang === 'qrcode') {
        // REVISI 2: Mengubah pl-4 menjadi justify-center pada col-nama dan col-ket
        thead.innerHTML = `
            <tr>
                ...
                <th class="hdr-std col-nama cursor-pointer hover:bg-slate-700 transition select-none" onclick="sortTable(${tabelSekarang==='hold_stbj'?13:12}, this)"><div class="flex items-center justify-center gap-1">Nama Item <i data-lucide="arrow-up-down" class="w-3 h-3 sort-icon opacity-50"></i></div></th>
                ...
                <th class="hdr-std col-ket cursor-pointer hover:bg-slate-700 transition select-none" onclick="sortTable(${tabelSekarang==='hold_stbj'?19:18}, this)"><div class="flex items-center justify-center gap-1">Keterangan <i data-lucide="arrow-up-down" class="w-3 h-3 sort-icon opacity-50"></i></div></th>
        ...
        // REVISI 1 & 2: PO Bawaan dan justify-center untuk Nama dan Keterangan
        thead.innerHTML = `
            <tr>
                <th class="hdr-std w-10 col-cb"><input type="checkbox" onchange="toggleSemuaCentang(this.checked)" class="cursor-pointer rounded text-blue-600"></th>
                ...
                <th class="hdr-std col-nama cursor-pointer hover:bg-slate-700 transition select-none" onclick="sortTable(12, this)"><div class="flex items-center justify-center gap-1">${isJasper ? 'Nama Barang Jasper' : 'Nama Item'} <i data-lucide="arrow-up-down" class="w-3 h-3 sort-icon opacity-50"></i></div></th>
                ...
                <th class="hdr-std border-r border-slate-500 col-po cursor-pointer hover:bg-slate-700 transition select-none" onclick="sortTable(17, this)"><div class="flex items-center justify-center gap-1">PO Bawaan <i data-lucide="arrow-up-down" class="w-3 h-3 sort-icon opacity-50"></i></div></th>
                ...
                <th class="hdr-std col-ket border-r border-slate-500 cursor-pointer hover:bg-slate-700 transition select-none" onclick="sortTable(19, this)"><div class="flex items-center justify-center gap-1">Keterangan <i data-lucide="arrow-up-down" class="w-3 h-3 sort-icon opacity-50"></i></div></th>
        ...
        arr.forEach((r, i) => {
            ...
            h += `
                <tr class="border-b border-slate-200 hover:bg-slate-50 text-row text-center transition text-xs">
                    ...
                    <td class="p-3 font-black col-status-data text-[10px]">${statData}</td>
                    ...
                    <td class="p-3 font-black text-orange-600 bg-orange-50/40 border-r border-slate-200 s-po col-po">${r.po}</td>
...
async function aksiMassal(tipe) {
    ...
    // REVISI 3: FUNGSI EXCEL MENANGKAP LANGSUNG APA YANG TERTAMPIL DI TABEL (CERDAS)
    else if(tipe === 'xlsx') {
        if(typeof XLSX === 'undefined') return alert("Library Excel belum termuat, pastikan ada koneksi internet.");
        
        let ws_data = [];
        
        // 1. Tarik Header yang saat ini sedang tampil (tidak di-hide)
        const headers = Array.from(document.querySelectorAll('#thead-stbj th'))
            .filter(th => window.getComputedStyle(th).display !== 'none' && !th.classList.contains('col-cb') && !th.classList.contains('col-btn'))
            .map(th => th.innerText.trim().replace(/\n/g, ' '));
        ws_data.push(headers);

        // 2. Tarik Data dari baris yang dicentang (Hanya kolom yang tampil)
        document.querySelectorAll('.row-cb:checked').forEach(cb => {
            const tr = cb.closest('tr');
            const rowData = [];
            Array.from(tr.children).forEach(td => {
