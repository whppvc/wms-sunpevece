...
window.bukaModalRiwayatKonversi = async function() {
    ...
    try {
        ...
        if(!data || data.length === 0) { 
            tbody.innerHTML = `<tr><td colspan="9" class="p-10 font-bold text-slate-400 text-center">Tidak ada riwayat konversi OUT.</td></tr>`; 
            return; 
        }
        ...
        data.forEach(d => {
            if(!grouped[d.kode_konversi]) {
                grouped[d.kode_konversi] = {
                    kode_konversi: d.kode_konversi,
            ...
            grouped[d.kode_konversi].items.push(d);
            grouped[d.kode_konversi].qty_total++;
        });

        riwayatKonversiList = Object.values(grouped);

        let h = '';
        riwayatKonversiList.forEach(d => {
            ...
            if(d.items.length > 0) {
                let item = d.items[0];
                detailItem = `<span class="text-blue-700">${item.nama_item}</span> | ${item.panjang} | ${item.grade} | ${item.dus} | ${item.shading}`;
                if(d.items.length > 1) detailItem += ` <br><span class="text-[10px] text-slate-400 font-black bg-slate-100 px-1 rounded">(+${d.items.length - 1} item lain)</span>`;
            }
            ...
            h += `
                <tr class="border-b hover:bg-slate-50 text-xs transition">
                    ...
                    <td class="p-3 font-medium text-slate-600 text-left whitespace-normal max-w-[200px] leading-tight border-r border-slate-200">${d.keterangan}</td>
        ...
        tbody.innerHTML = `<tr><td colspan="9" class="p-5 text-red-500 text-center">${e.message}</td></tr>`; 
    }
...
window.lihatDetailKonversi = function(kode) {
    ...
    items.forEach((item, idx) => {
        t += `<tr class="border-b border-slate-100 text-center hover:bg-white bg-slate-50/50">
                ...
                <td class="p-2 text-left font-semibold text-blue-600">${item.nama_item}</td>
                <td class="p-2 font-medium text-slate-700">${item.panjang}</td>
