...
// REVISI 3: Variabel untuk Filter & Paginasi
...
document.addEventListener('DOMContentLoaded', async () => { 
    ...
    // REVISI 3: Listener untuk close Filter Menu
    ...
    await loadAreas(); 
    window.renderHeaderTabel(); // REVISI 3: Inisialisasi Header
});
...
window.setModeKonversi = function(mode) {
    ...
    dataPic = []; 
    window.activeFilters = {}; // Reset filter saat ganti tab
...
// ==========================================
// SCANNING HANDLER (REVISI 1: Disatukan)
// ==========================================
...
// ==========================================
// REVISI 3: HEADER FILTER EXCEL PRO & SORT
// ==========================================
...
window.renderTablePic = function() {
    ...
    dataPic.forEach((d, index) => {
        ...
        if (d.status === 'VALID' && d.poAktualUI !== '-' && d.poAktualUI !== 'KOSONG') {
    ...
    window.saringTabelExcel(); // Terapkan filter & paginasi
};
        ...
        if (currentMode === 'out' || currentMode === 'pindah') {
            dataPic.forEach(d => {
                ...
                if (matched) {
                    ...
                    let baseSpec = `${d.namaItem}_${d.panjang}_${d.grade}_${d.dus}_${d.shading}`;
